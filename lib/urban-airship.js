 ...
 */
UrbanAirship.prototype.getDeviceTokenStatus = function(since, callback) {
        this._auth = new Buffer(this._key + ":" + this._master, "utf8").toString("base64");
        this._transport("/api/device_tokens/feedback/?since="+since, "GET", function(error, response_data) {
 ...
 * @params callback(error, data)
 */
UrbanAirship.prototype.getApidsStatus = function(since, callback) {
        this._auth = new Buffer(this._key + ":" + this._master, "utf8").toString("base64");
        this._transport("/api/apids/feedback/?since="+since, "GET", function(error, response_data) {
                callback(error, response_data || {});
        });
}
